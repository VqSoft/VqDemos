...
namespace WinDemo
{
    public static class StringExtensions
    {
        ...
        ///; \\ encodes \, \N or \n encodes newline
        ///; \; encodes ;, \, encodes ,
        ///reference: http://www.ietf.org/rfc/rfc2445.txt 4.3.11 Text
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        public static string IcsEscapedChar(this string text)
        {
            return string.IsNullOrEmpty(text) ? text : text.Replace(@"\", @"\\").Replace(@",", @"\,").Replace(@";", @"\;");
        }
        ...
        public static decimal DecimalValue(this string s)
        {

            decimal returnValue = decimal.MinValue;
            decimal.TryParse(s, out returnValue);
            return returnValue;
        }
        ...
        public static string ToCurrency(this decimal? input)
        {
            ...
            decimal inputValue = decimal.Parse("0.00");
            ...
            // Assumes the current culture is been set
            CultureInfo us = CultureInfo.CurrentCulture;
            return string.Format(us, "{0:C}", inputValue); ;
        }
        ...
        public static string ToCurrency(this decimal input)
        {
            ...
            CultureInfo us = CultureInfo.CurrentCulture;
            return string.Format(us, "{0:C}", input); ;
        }

        public static string ToCurrency(this decimal? input, string culture)
        {
            ...
            CultureInfo us = CultureInfo.GetCultureInfo(culture);
            return string.Format(us, "{0:C}", inputValue); ;
        }

        public static string WithHttp(this string input)
        {
        ...
        public static bool IscontainJavaScrip(this string input)
        {
            List<Regex> lsRegex = new List<Regex>
            {
                new Regex(@"<[\s]*?script[\s\S]+<[\s]*?/script[\s]*?>",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase),
                new Regex(@"javascript[\s]*?:[\S]*?\(\w*?\)[\;]*?",
                    System.Text.RegularExpressions.RegexOptions.IgnoreCase),
                new Regex(@"on[\S]*?[\s]*?=", System.Text.RegularExpressions.RegexOptions.IgnoreCase)
            };

            return lsRegex.Any(t => t.IsMatch(input));
        }
        #endregion
